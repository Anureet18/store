...
const Store = () => {
    ...
    function handleEdit(id, index) {
        const edit = items.find((ele) => ele.id === id)
        setName(edit.name)
    ...
    const handleUpdate = () => {
        if (name !== '' && quantity !== '' && price !== "") {
            setItems((previtems) => {
                ...
                updateditem[edit] = { ...updateditem[edit], name, quantity };
    ...
    function handlePlus(id) {
        const result = items.find((ele) => ele.id === id)
        result.quantity++
                        ...
                        {items?.map((data, index) => {
                            ...
                            return (
                                <tr key={data.id}>
                                    <td >{index + 1}</td>
                                    <td>{data.name}</td>
                                    <td><Button variant="success" onClick={() => handlePlus(data.id)}>Plus</Button>
                                        {data.quantity}
                                        <Button variant="success" onClick={() => handleMinus(data.id)}>Minus</Button>
                                    </td>
                                    <td><Button variant="success" onClick={() => addPrice(data.id)}>Plus</Button>
                                        {data.price}
                                        <Button variant="success" onClick={() => minusPrice(data.id)}>Minus</Button>
                                    </td>
                                    <td>₹{totalPrice}</td>
                                    <td>
